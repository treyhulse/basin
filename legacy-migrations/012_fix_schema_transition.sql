-- Migration 012: Fix schema transition and create missing data tables
-- This migration fixes the create_data_table function and creates missing data tables

-- First, let's fix the create_data_table function to use proper parameter names
...
-- Also fix the drop_data_table function to use proper parameter names
DROP FUNCTION IF EXISTS drop_data_table(UUID, TEXT);

CREATE OR REPLACE FUNCTION drop_data_table(p_collection_id UUID, p_table_name TEXT)
RETURNS VOID AS $$
DECLARE
    tenant_schema TEXT;
BEGIN
    ...
    EXECUTE 'DROP TABLE IF EXISTS "' || tenant_schema || '".data_' || p_table_name;
END;
$$ LANGUAGE plpgsql;
